...
Pacificsoft server nodes act as an MQTT Broker that supports QoS levels 0 (at most once) and 1 (at least once) and a set of predefined topics.
...
* **0x00 Connected** - Successfully connected to Pacificsoft MQTT server.
* **0x04 Connection Refused, bad user name or password** - Username is empty.
* **0x05 Connection Refused, not authorized** - Username contains invalid **$ACCESS_TOKEN**. 

## Key-value format

By default, Pacificsoft supports key-value content in JSON. Key is always a string, while value can be either string, boolean, double or long.
...
In order to publish telemetry data to Pacificsoft server node, send PUBLISH message to the following topic:
