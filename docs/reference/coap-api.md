...
Pacificsoft server nodes act as a CoAP Server that supports both regular and observe requests.
...
In order to publish telemetry data to Pacificsoft server node, send POST request to the following URL:
...
In order to publish client-side device attributes to Pacificsoft server node, send POST request to the following URL:
...
In order to request client-side or shared device attributes to Pacificsoft server node, send GET request to the following URL:
