...
The Gateway is a special type of device in Pacificsoft that is able to act as a bridge between external devices connected to different systems and Pacificsoft.
Gateway API provides the ability to exchange data between **multiple devices** and the platform using **single MQTT connection**.
The Gateway also acts as a Pacificsoft device and can leverage existing [MQTT Device API](/docs/reference/mqtt-api/) to report stats, receive configuration updates and much more.

The API listed below is used by [**Pacificsoft open-source IoT Gateway**](/docs/iot-gateway/what-is-iot-gateway/).
...
In order to inform Pacificsoft that device is connected to the Gateway, one needs to publish following message:
...
Once received, Pacificsoft will lookup or create a device with the name specified.
Also, Pacificsoft will publish messages about new attribute updates and RPC commands for a particular device to this Gateway.

## Device Disconnect API

In order to inform Pacificsoft that device is disconnected from the Gateway, one needs to publish following message:
...
Once received, Pacificsoft will no longer publish updates for this particular device to this Gateway.

## Attributes API

Pacificsoft attributes API allows devices to
...
In order to publish client-side device attributes to Pacificsoft server node, send PUBLISH message to the following topic:
...
In order to request client-side or shared device attributes to Pacificsoft server node, send PUBLISH message to the following topic:
...
In order to publish device telemetry to Pacificsoft server node, send PUBLISH message to the following topic:
