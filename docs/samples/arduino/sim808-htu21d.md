...
description: Pacificsoft IoT Platform sample for humidity and temperature data upload over HTTP using Arduino UNO, SIM808 GSM shield and HTU21D sensor.
...
Collected data is pushed via HTTP to Pacificsoft server for storage and visualization.
...
Arduino UNO pushes data to Pacificsoft server via HTTP protocol by using [Arduino Pacificsoft SDK](https://github.com/thingsboard/ThingsBoard-Arduino-MQTT-SDK).
...
### Step 2. Install Arduino Pacificsoft SDK and dependencies
...
- `THINGSBOARD_SERVER` - Pacificsoft HOST/IP address that is accessible from within your wifi network. Specify "demo.thingsboard.io" if you are using [live demo](https://demo.thingsboard.io/) server.
...
After application will be uploaded and started it will try to connect to Pacificsoft node using HTTP and upload "humidity" and "temperature" timeseries data once per second.
